...
function ChartCard({ icon: Icon, title, data, dataKeys, accent, unit, onClick }: ChartCardProps) {
  const gid = `vl-${title.replace(/[^a-z]/gi, '')}`;
  const dual = dataKeys.length > 1;
  return (
    <button
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <div className="v-chip" style={{ width: 30, height: 30, borderRadius: 10, background: `${accent}24`, color: accent }}>
        ...
        <ResponsiveContainer width="100%" height={130} minWidth={0}>
          {dual ? (
            <LineChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -22 }}>
              <CartesianGrid strokeDasharray="3 3" stroke={AXIS} strokeOpacity={0.16} vertical={false} />
              <XAxis dataKey="time" stroke={AXIS} tick={{ fontSize: 8, fill: AXIS }} tickLine={false} axisLine={false} minTickGap={24} />
              <YAxis stroke={AXIS} tick={{ fontSize: 8, fill: AXIS }} tickLine={false} axisLine={false} unit={unit} />
              <Line type="monotone" dataKey={dataKeys[0]} stroke={accent} strokeWidth={2} dot={false} isAnimationActive={false} />
              <Line type="monotone" dataKey={dataKeys[1]} stroke="var(--good)" strokeWidth={1.6} dot={false} isAnimationActive={false} />
            </LineChart>
          ) : (
            <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: -22 }}>
              ...
              <XAxis dataKey="time" stroke={AXIS} tick={{ fontSize: 8, fill: AXIS }} tickLine={false} axisLine={false} minTickGap={24} />
              <YAxis stroke={AXIS} tick={{ fontSize: 8, fill: AXIS }} tickLine={false} axisLine={false} unit={unit} />
              <Area type="monotone" dataKey={dataKeys[0]} stroke={accent} strokeWidth={2} fill={`url(#${gid})`} isAnimationActive={false} />
            </AreaChart>
          )}
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="v-screen">
      ...
      <div className="v-segment">
        <button data-active={is24} onClick={() => setTimeRange('24h')}>Letzte 24 Stunden</button>
        <button data-active={!is24} onClick={() => setTimeRange('7d')}>Letzte 7 Tage</button>
      </div>
      ...
      >
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--brand)' }}>Umfassende Gesundheitsanalyse</span>
      </button>

      {/* Charts */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        <ChartCard icon={Heart} title="Herzfrequenz" data={is24 ? hrData24h : hrData7d} dataKeys={['value']} accent={VITAL_COLOR.heart} unit=" BPM" onClick={() => onVitalClick('heart')} />
        <ChartCard icon={Activity} title="Blutdruck" data={is24 ? bpData24h : bpData7d} dataKeys={['systolic', 'diastolic']} accent={VITAL_COLOR['blood-pressure']} unit=" mmHg" onClick={() => onVitalClick('blood-pressure')} />
        <ChartCard icon={Droplets} title="Sauerstoffsättigung (SpO₂)" data={is24 ? spo2Data24h : spo2Data7d} dataKeys={['value']} accent={VITAL_COLOR.oxygen} unit=" %" onClick={() => onVitalClick('oxygen')} />
        <ChartCard icon={Thermometer} title="Körpertemperatur" data={is24 ? tempData24h : tempData7d} dataKeys={['value']} accent={VITAL_COLOR.temperature} unit=" °C" onClick={() => onVitalClick('temperature')} />
        <ChartCard icon={Zap} title="EKG" data={is24 ? ekgData24h : ekgData7d} dataKeys={['value']} accent={VITAL_COLOR.ekg} unit=" BPM" onClick={() => onVitalClick('ekg')} />
        <ChartCard icon={Flame} title="Kalorienverbrauch" data={is24 ? caloriesData24h : caloriesData7d} dataKeys={['value']} accent={VITAL_COLOR.calories} unit={is24 ? ' kcal/h' : ' kcal'} onClick={() => onVitalClick('calories')} />
